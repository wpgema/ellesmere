...
<!--begin::App Content Header-->
    <div class="app-content-header">
        ...
        <!--begin::Row-->
        <div class="row">
            <div class="col-sm-6"><h3 class="mb-0">Laporan Pembelian Bulanan</h3></div>
            <div class="col-sm-6">
            <ol class="breadcrumb float-sm-end">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Laporan Pembelian Bulanan</li>
            </ol>
            </div>
        </div>
        <!--end::Row-->
    ...
    <div class="app-content">
        ...
        <div class="container">
            <div class="row justify-content-center mb-3 p-3 bg-white">
                <div class="col-lg-12">
                    <canvas id="myChartPembelian"></canvas>
                    <table class="table mt-3">
                        ...
                        <tbody>
                            <?php $i = 1; foreach($pembelian as $pmbln) : ?>
                                <tr class="align-middle">
                                    <td style="width: 10px"><?= $i; ?></td>
                                    <td><?= $pmbln["bulan"] ?></td>
                                    <td>Rp. <?= number_format($pmbln["total_price"]) ?></td>
                                    <td>
                                        <a href="<?= base_url()  ?>laporan-pembelian/<?= $pmbln["bulan_angka"] ?>" class="border-0 my-1 btn btn-sm btn-primary">Detail</a>
                                    </td>
                                </tr>
                            <?php $i++; endforeach; ?>
                        </tbody>
    ...
    <script>
        ...
        function getPembelian(){
            $(document).ready(function () {
                ...
                const phpData = <?php echo json_encode($pembelian); ?>;
                // Fungsi untuk memuat data menggunakan AJAX
                let pembelian = collect(phpData).map(function(item){
                    return item.total_price
                }).all();
                let bulan = collect(phpData).map(function(item){
                    return item.bulan
                }).all();
                const ctx = document.getElementById('myChartPembelian');
                new Chart(ctx, {
                    type: 'line',
                    data: {
                    labels: bulan,
                    datasets: [{
                        label: 'Tren Pembelian',
                        data: pembelian,
                        borderWidth: 2,
